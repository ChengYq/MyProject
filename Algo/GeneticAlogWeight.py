...
def b2d(b):  # 将二进制转化为十进制 x∈[0,10]
    ...
    g = 0.00001 + tempg * (1 - 0.00001) / (2 ** 10 - 1)
...
def calobjvalue(pop, y, x, yt, xt, W):  # 计算目标函数值
    tempC = []
    tempg = []
    objvalue = []
    for i in range(len(pop)):
    ...
    for i in range(len(tempC)):
        ...
        g = 0.00001 + tempg[i] * (1 - 0.00001) / (2 ** 10 - 1)
        print '-------------'
        print c, g
...
def genetic(x, y, xt, yt, W):
    ...
    for i in range(100):  # 繁殖100代
        ...
        objvalue = calobjvalue(pop, y, x, yt, xt, W)  # 计算目标函数值
